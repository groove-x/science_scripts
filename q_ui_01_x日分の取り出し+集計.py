...
class AnalysisApp:
    ...
    def run_analysis(self):
        ...
        for csv_file in diary_csv_files:
            ...
            # DataFrameBの特定の列（この場合は 'Data'）だけを選択してマージ
            df_b_selected = count_days[['q_Date+GhostID', 'count', 'Event']]  # 'A' 列と 'Data' 列を選択
            df_result = pd.merge(df_score, df_b_selected, on='q_Date+GhostID', how='left')
            # 'count' 列の NaN を 0 に置き換える
            df_result['count'] = df_result['count'].fillna(0)
            df_result.to_csv("{}/{}days_HBT_sum_{}.csv".format(out_folder, cal_days, csv_file), index=False)  # index=False により、インデックスはCSVに含まれない
            ...
            if "HUGGED_"in csv_file:
                ...
                # 'count' 列の NaN を 0 に置き換える
                df_result['count'] = df_result['count'].fillna(0)
                df_result.to_csv("{}/{}days_sum_duration_{}.csv".format(out_folder, cal_days, csv_file), index=False)  # index=False により、インデックスはCSVに含まれない
