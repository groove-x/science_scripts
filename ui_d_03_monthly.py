...
class AnalysisApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ui_d_03_monthly")
        self.input_folder = ""
        self.output_folder = ""
    ...
    def run_analysis(self):
        ...
        for csv_file in diary_csv_files:
            ...
            if "usedays"in csv_file:
                unique = df_data.groupby(['GhostID', '30days_unit', 'BirthDate', 'score'])["counts"].size().reset_index().rename(columns={"counts":"30days_sum"})
            else:
                unique = df_data.groupby(['GhostID', '30days_unit', 'BirthDate', 'score'])["sum_duration"].size().reset_index().rename(columns={"sum_duration":"30days_sum"})

            # 別列の数値を日数として追加する
            unique['30days_EventDate'] = unique['BirthDate'] + pd.to_timedelta(unique['30days_unit'], unit='d')

            # 各GhostIDについて変化量を計算し、新しい列 'change' を作成する
            unique['change_360'] = unique.groupby('GhostID').apply(lambda x: calculate_change(unique, x.name, 360)).reset_index(level=0, drop=True)
            # 各GhostIDについて変化量を計算し、新しい列 'change' を作成する
            unique['change_0'] = unique.groupby('GhostID').apply(lambda x: calculate_change(unique, x.name, 0)).reset_index(level=0, drop=True)

            csv_file=csv_file[:-4]
            unique.to_excel("{}/monthly_{}.xlsx".format(out_folder, csv_file))
