...
class AnalysisApp:
    def __init__(self, root):
        ...
        # フィルタリングに使う文字を入力するテキストフィールド
        self.filter_label2 = tk.Label(root, text="Enter group Event2 name:")
        self.filter_label2.pack(pady=5)

        self.filter_entry2 = tk.Entry(root)
        self.filter_entry2.pack(pady=5)

        # グループを分ける値
        self.filter_label_g2 = tk.Label(root, text="Enter g2-value:")
        self.filter_label_g2.pack(pady=5)

        self.filter_entry_g2 = tk.Entry(root)
        self.filter_entry_g2.pack(pady=5)
    ...
    def run_analysis(self):
        ...
        filter_event_name2 = self.filter_entry2.get().strip()
        g2_value = self.filter_entry_g2.get().strip()
        # floatに変換する
        g2_value = float(g2_value)  # もしくはfloat(cal_days)
        ...
        columns =["p_value", "statistic",
                "p_H1H2-H1L2", "effect_r_H1H2-H1L2",
                "p_H1H2-L1L2", "effect_r_H1H2-L1L2",
                "p_H1H2-L1H2", "effect_r_H1H2-L1H2",
                "p_H1L2-L1L2", "effect_r_H1L2-L1L2",
                "p_H1L2-L1H2", "effect_r_H1L2-L1H2",
                "p_L1L2-L1H2", "effect_r_L1L2-L1H2",
                "H1H2_n", "H1L2_n", "L1H2_n", "L1L2_n", 
                "H1H2_median", "H1L2_median", "L1H2_median", "L1L2_median", 
                "H1H2_mean", "H1L2_mean", "L1H2_mean", "L1L2_mean", 
                "H1H2_std", "H1L2_std", "L1H2_std", "L1L2_std"
                  ]
        ...
        df_score.loc[(df_score[filter_event_name1] >= g1_value) & (df_score[filter_event_name2] >= g2_value), 'group'] = 'H1H2'  # グループ1
        df_score.loc[(df_score[filter_event_name1] >= g1_value) & (df_score[filter_event_name2] <  g2_value), 'group'] = 'H1L2' # グループ2
        df_score.loc[(df_score[filter_event_name1] <  g1_value) & (df_score[filter_event_name2] >= g2_value), 'group'] = 'L1H2' # グループ3
        df_score.loc[(df_score[filter_event_name1] <  g1_value) & (df_score[filter_event_name2] <  g2_value), 'group'] = 'L1L2' # グループ4


        for event in filtered_events:
            # Kruskal-Wallis検定の実施# グループごとのスコアをリストに格納
            group_H1H2 = df_score[df_score['group'] == 'H1H2'][event]
            group_H1L2 = df_score[df_score['group'] == 'H1L2'][event]
            group_L1H2 = df_score[df_score['group'] == 'L1H2'][event]
            group_L1L2 = df_score[df_score['group'] == 'L1L2'][event]
            kruskal_result = stats.kruskal(group_H1H2, group_H1L2, group_L1H2, group_L1L2)

            # 検定統計量とp値を取得
            statistic = kruskal_result.statistic
            p_value = kruskal_result.pvalue
            df_result.loc[event, "p_value"] = p_value
            df_result.loc[event, "statistic"] = statistic
        ...
        print(df_result)
        df_result.to_csv("{}/群間検定_{}{}_{}{}.csv".format(out_folder, filter_event_name1, g1_value, filter_event_name2, g2_value))  # index=False により、インデックスはCSVに含まれない
